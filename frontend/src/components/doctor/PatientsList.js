...
function PatientsList() {
  ...
  const handleViewDetails = async (patientId) => {
    try {
      const response = await fetch(`http://k8s-default-backends-848a823787-ea2281742964f835.elb.us-east-2.amazonaws.com:8000/api/doctor/patient/${patientId}/details`);
      const data = await response.json();
  ...
  const handleViewSummary = async (patientId) => {
    try {
      const response = await fetch(`http://k8s-default-backends-848a823787-ea2281742964f835.elb.us-east-2.amazonaws.com:8000/api/doctor/patient/${patientId}/reports`);
      const data = await response.json();
    ...
    <>
      ...
      <Dialog open={!!selectedReportId} onClose={() => setSelectedReportId(null)} maxWidth="lg" fullWidth>
        ...
        <DialogContent>
          {selectedReportId && (
            <iframe
              src={`http://k8s-default-backends-848a823787-ea2281742964f835.elb.us-east-2.amazonaws.com:8000/api/reports/view/${selectedReportId}`}
              width="100%"
              height="600px"
              title="PDF Viewer"
            />
